        ...
        function(cb){
            if(_.has(self.options, 'interface')){
                network.get_interfaces_list(function(err, interfaces){
                    if(_.isNull(err)){
                        var interface = _.find(interfaces, { name : self.options.interface });
                        if(!_.isUndefined(interface))
                            address_config.private = interface.ip_address;
                    }
    ...
    async.each(targets, function(target, fn){
        ...
        socket.connect(target.port, target.address[scope], function(){
            ...
            if(_.contains(unencryptable_events, json.event)){
                ...
                crypto.encrypt(encryption_options, function(error, encryption){
        ...
        if(json.stream){
            var queue = async.queue(function(encryption_options, fn){
                var client = encryption_options.client;
...
Network.prototype.generate_node_id = function(fn){
    crypto.generate_node_id(function(error, id){
        if(!_.isUndefined(id))
            id = id.toString('hex');
